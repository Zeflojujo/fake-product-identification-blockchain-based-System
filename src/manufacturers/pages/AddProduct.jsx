...
import { addProduct } from '../../BlockchainService'
import Product from "../../assets/red-wine.jpg"
...
const AddProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!product.productName || !product.description || !product.qrHash || !product.blodkId) return

        setGlobalState('modal', 'scale-0')
        setGlobalState('loading', { show: true, msg: 'Adding product...' })

        try {

            setLoadingMsg('Initializing transaction...')
            const result = await addProduct({ qrHash: product.qrHash, blockId: product.blodkId, productName: product.productName, description: product.description })

            if (result) {
                resetForm()
                setAlert('Product added successfully..!', 'green')
            } else {
                throw Error
            }
            ...
            <div className="flex flex-col w-full gap-y-4 text-3xl semibold h-full">
                ...
                <div className="flex flex-col justify-center items-center mx-auto w-full">
                    ...
                    <div className="flex flex-col justify-center items-center mx-1 md:mx-4 overflow-hidden z-0 my-11 w-full">
                        {/* Manufacturers productTable component is included */}
                        <div className="w-4/5">
                            <button
                            ...
                            <form className="flex flex-col" onSubmit={handleSubmit}>
                                ...
                                <div className="mt-4">
                                    <input
                                        ...
                                        name="blodkId"
                                        placeholder="blodkId"
                                        onChange={(e) => setProduct({ ...product, blodkId: e.target.value })}
                                        value={product.blodkId}
                                        required
                                    />
                                </div>
                                ...
                                <div className="mt-4">
                                    <input
                                        ...
                                        name="qrHash"
                                        placeholder="qrHash"
                                        onChange={(e) => setProduct({ ...product, qrHash: e.target.value })}
                                        value={product.qrHash}
                                        required
                                    />
                                </div>
